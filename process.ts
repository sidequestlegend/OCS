...
export class Process{
    ...
    async run(data: ScrapeData):Promise<ProcessedData>{
        return new Promise((resolve, reject)=>{
            this.db.all("SELECT id, display_name, canonical_name, price FROM oculus_quest_store;", (err, rows)=> {
                ...
                data.data.node.all_items.edges.forEach(item=>{
                    ...
                    if(existing_item) {
                        let lowerPrice = Number(existing_item.price) > Number(item.node.current_offer.price.offset_amount);
                        let lowerPriceCrossBuy = item.rift_cross_buy && Number(existing_item.cross_buy_price) > Number(item.rift_cross_buy.node.current_offer.price.offset_amount);
